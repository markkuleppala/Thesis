...
Kata Containers provides the workload with the guest kernel. Meanwhile, the host uses a dedicated kernel, which are not connected. One benefit of the architectural change compared to standard container workloads is the availability in case of kernel crashes. This availability is highly beneficial for logging purposes, as the crash of the worker container does not wipe all logs at the same time.
